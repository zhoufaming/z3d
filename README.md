...
1. **导入**：点击左上角「导入 3MF」选择 `.3mf` 文件；「追加合并」可继续叠加其它 3MF
...
| `npm run test` | 生成样例 + 运行端到端回归测试（60 项） |
...
`tools/test-roundtrip.mjs` 覆盖读 → 改 → 写 → 再读的 60 项断言：几何无损、transform 往返一致、料槽颜色保持、组合件零件偏移保持、三角面总数无损等。
